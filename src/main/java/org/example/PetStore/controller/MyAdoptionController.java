...
public class MyAdoptionController {
    ...
    public Dict adopt(@RequestParam("address") String address, @RequestParam("petId") Integer petId){
        return myAdoptionService.adopt(address, petId);
    }

    @ApiOperation(value = "列出宠物被领养的情况列表", notes = "列出宠物被领养的情况列表")
    @RequestMapping(value = "listPetAdoption", method = RequestMethod.GET)
    public Dict listPetAdoption(@RequestParam("address") String address){
        return myAdoptionService.listPetAdoption(address);
    }
