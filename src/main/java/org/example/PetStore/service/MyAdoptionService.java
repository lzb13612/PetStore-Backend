...
public class MyAdoptionService {
    ...
    public Dict adopt(String userAddress, Integer petId) {
        /**
         * 假设有8个宠物可供领养，编号为 0-7
         * 输入宠物编号为 0-7 之间，若不在此范围内则返回 code 为 500
         * */
        if(petId < 0 || petId > 7){
        ...
        /**
         * 获取返回结果，并解析，如果结果与输入宠物编号相符则返回 200
         * */
    ...
    public Dict listPetAdoption(String userAddress) {
         ...
         * 获取领养宠物的账户地址列表
         * */
        JSONArray resArr2 = getPetAdoptAddress(userAddress);
        List<Integer> petStatusList = new ArrayList<>();
        /**
         * 解析账户，若为默认地址(0x0000000000000000000000000000000000000000)则表示该编号宠物没有领养
         * */
    ...
    /**
     * 通过地址获取宠物列表，如果获取地址列表不是查询用户，则说明该用户未领养此宠物
     ...
     * 通过用户地址获取领养宠物的地址列表
     * */
    public JSONArray getPetAdoptAddress(String userAddress){
        List<String> funcParam = new ArrayList<>();
        Dict result = weBASEUtils.commonReq(
                userAddress, "getAdopters", funcParam, ABI, "Adoption", contractAddress
        );
